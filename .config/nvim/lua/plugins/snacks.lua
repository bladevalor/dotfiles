            ...
            actions = {
              explorer_paste = function(picker, item) --[[Override]]
                ...
                files = vim.tbl_filter(function(file)
                  ...
                  -- NOTE: `vim.uv.fs_stat` is more robust and hanldes both `files` and `directories`
                ...
                -- NOTE: Prefer using the parent folder if `dir` is closed/collpased
                if item.dir and not item.open then
                  dir = vim.fs.dirname(dir)
                end

                for _, file in ipairs(files) do
                  -- BUG: Prevent pasting inside itself (i.e A recursive copy)
                  if file == dir then
